...
const Home = () => {
  ...
  const handleShowTexts = () => {
    const { volume, activation, voices } =
  ...
  useEffect(() => {
    const fetchGlobalConfig = async () => {
      if ((globalThis as any).ipc) {
        const globalConfig =
          (await (globalThis as any).ipc.tobiiInControl()) || false;
  ...
  return (
    <div
      id="home"
      className="flex justify-between w-full h-screen bg-zinc-900 relative"
    >
      {showModal && (
        <ModalConfig configuration={config} closeModal={closeModal} />
      )}
      {modalProps !== null && (
        <ConfirmationModal
      ...
      <div className="flex justify-start w-full px-20 py-10 relative">
        ...
        <div className="absolute right-4 top-4 flex items-center gap-4">
          <ButtonAnimation
            functionKeyboard={{
              funct: "calibrateTobiiEyetracker",
              state: changeState,
            }}
          ...
          <ButtonAnimation
            functionKeyboard={{
              funct: "toggleTobiiEyetracker",
              state: changeState,
            }}
            speakText="Tobii"
            text="Activar eyetracker 2"
            propClass="w-[100px] h-[80px]"
