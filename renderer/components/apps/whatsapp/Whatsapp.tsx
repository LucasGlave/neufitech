...
const Whatsapp = () => {
  ...
  const [activeButtonConfigs, setActiveButtonConfigs] = useState<
  ...
  const handleActive = (func: string) => {
    ...
    const route = whatsappRoutes.find(
      (route) => Object.keys(route)[0] === func
    ) as WhatsappRoute | undefined;
        ...
        <div className="flex flex-col items-center justify-between p-2 w-[20%] h-full">
          ...
          <div className="p-2 flex flex-col gap-2 w-full">
            {activeButtons.map((buttonName, index) => {
              ...
              const route = whatsappRoutes.find(
                (route) => Object.keys(route)[0] === buttonName
              ) as WhatsappRoute | undefined;
              return (
                <ButtonAnimation
                  disabled={isOff ? true : false}
                  ...
                  keyCombination={
                    !buttonConfig.keyCombination
                      ? route && route[buttonName].keyCombination
                      : buttonConfig.keyCombination
                  }
                  keyPress={
                    !buttonConfig.keyPress
                      ? route && route[buttonName].keyPress
                      : buttonConfig.keyPress
                  }
                  tabLoop={
