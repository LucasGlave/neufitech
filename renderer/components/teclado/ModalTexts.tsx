...
const ModalTexts = ({ activeTexts, state, setter }: ModalTextsProps) => {
    ...
    return (
        <div id="ScrollText" className="absolute items-start justify-start bg-[#000000ea] gap-5 flex flex-col w-full min-h-screen p-16 z-50">
            {modalProps !== null && (
                <ConfirmationModal
            ...
            <div className="w-[85%] pr-2 flex items-center justify-between">
                <ButtonAnimation
                    disabled={isOff}
                ...
                <ButtonAnimation
                    ...
                    } propClass="w-[200px] h-[80px] bg-keybackground"
                />
            </div>
            <div className="flex items-start justify-center h-full w-full gap-2">
                ...
                <Scroll isOff={isOff} maxScrollValue={scrollMax} bgcolor={true} deleteFunction={handleDeleteTexts} />
            </div>
        </div>
    )
}
